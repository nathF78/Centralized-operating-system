    ...
    if (strcmp(cmd->seq[0][0], "cd") == 0) {
      ...
      if ((cmd->seq[1]) != NULL) {
        ...
        if (pidFils < 0) { //erreur du fils
          ...
          if (close(pipe[0]) == -1) { //on ferme l'extremité inutilisée
            perror("error close"); exit(7); 
          }
          if (dup2(pipe[1], 1) == -1) { //on redirige 
            perror("error dup2"); exit(8); 
          }
          if (close(pipe[1]) == -1) { //on referme l'extrémité 
            perror("erreur close"); exit(7); 
          }
        }

        printf("woaw 2 commandes");
      }
      ...
      pidFils = fork();
        if (pidFils < 0) { //erreur du fils
          ...
          Processus p = newProcessus(&liste_p, pidFils, ACTIVE, cmd->seq[0][0]);
          add(&liste_p, p);
